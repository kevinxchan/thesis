...
while read name; do
	...
	for f in *.sam; do
		...
		samtools view -b -F 4 $filename.sam | samtools calmd -S - $WORK_DIR/references/fungene_9.5.1_recA_nucleotide_uclust99.fasta | samtools view -bS - > $out
	done
done < $dataset_names
...
while read name; do
	...
	for f in *_with_md.bam; do
		...
		samtools sort -o $filename $f && samtools index $filename
	done
done < $dataset_names
